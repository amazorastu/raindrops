...
Action* RelaxMap::creatDropAnimation(Point targetPoint, float height)
{
...
float RelaxMap::getHalfMaxWidth()
{
	return spaceWidth * (m_MapSize + 1)*0.5f;
}
RelaxMap::RelaxMap(Node* showNode, float w, float h)
{
	m_ShowNode = showNode;
...
Point RelaxMap::getDropPos(int x, int y)
{
...
DropsBase* RelaxMap::getDrop(int x, int y){
	return map[x][y];
}

int RelaxMap::isInMap(float touchPositionX, float touchPositionY)
{
	int x, y;
...
bool RelaxMap::isNear(int x1, int y1, int x2, int y2)
{
...
void RelaxMap::DragOn(float touchPositionX, float touchPositionY)
{
...
void RelaxMap::DragUp(float touchPositionX, float touchPositionY)
{
	if (dropsLinked.size() == 1)
	{
		if (map[dropsLinked.top().x][dropsLinked.top().y])
...
bool RelaxMap::getIsRound()
{
	return isRound;
}
Color4B RelaxMap::getCurrentColor()
{
	return currentColor;
}

void RelaxMap::clearScore(){
	m_Score = 0;
}
int RelaxMap::getScore(){
	return m_Score;
}